...
while on_off:
    ...
    if listen: # Falls Sound wahrgenommen wurde
    ...
    while listen:
        ...
        if ergebnis == "marvin":
            ...
            while active:
                ...
                record(command_FILENAME)
                process(command_FILENAME)
                command = test_realtime(command_FILENAME)
                print(command)
                if command != "background":  # Ich brauche noch viele leere Background dateien
                    if command == "weather":
                        print(weather_values("Wiesbaden",1)[1])
                        active = False
                        listen = False
                        ...
                        active = False
                        listen = False
                        # Happy --> hyppy music  ( Zahl für Lautstärke)
